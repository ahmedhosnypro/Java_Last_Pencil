...
public class LastPencilTest extends StageTest<String> {
    ...
    CheckResult checkOut() {//all comments correspond to before parts of tests
        ...
        if (lines.length != 1 || !Pattern.matches("^(how many pencils would you like to use)\\??:?$", lines[0])) {
            throw new WrongAnswer("When the game starts, it should output only one line asking the user about the " +
        ...
        if (output2.split("\\n").length != 1) {
            throw new WrongAnswer("When the user replies with the amount of pencils, the game should print 1 non-empty "
        ...
        if (!Pattern.matches(whoWillBeRegex, output2))
            throw new WrongAnswer("Discrepancy with the task, pay attention to the line " +
                    "\"Who will be the first\"");
        if (!Pattern.matches(playerChecking, output2))
            throw new WrongAnswer("Discrepancy with the task, output example" +
...
//all comments correspond to before parts of tests
    CheckResult checkNumericAmount(String input) {
        ...
        if (!output.contains("number of pencils") || !output.contains("numeric")) {
            throw new WrongAnswer("When the user provides the number of pencils as a non-numeric sequence, the game should "
    ...
    CheckResult checkZero() {//all comments correspond to before parts of tests
        ...
        if (!output.contains("number of pencils") || !output.contains("positive")) {
            throw new WrongAnswer("When the user provides \"0\" as a number of pencils, "
    ...
    CheckResult checkBothAmount() {//all comments correspond to before parts of tests
        ...
        String output = main.execute("0").toLowerCase();
        if (!output.contains("number of pencils") || !output.contains("positive")) {
            throw new WrongAnswer("When the user provides \"0\" as a number of pencils, the game should "
                    + "inform the user that their input is incorrect and prompt the user for input again "
        ...
        if (!Pattern.matches(".*\\([a-zA-Z_0-9]+,[a-zA-Z_0-9]+\\):?", output2)) {
            throw new WrongAnswer("When the user inputs the number of pencils correctly," +
    ...
    @DynamicTest(data = "test_data", repeat = 10)
        // added new option "repeat-10" for tests stability support
    CheckResult checkResult(int amount, int first, String[] incorrect) {
        ...
        if (leftName.equals(rightName)) { //Checking player name1 != name2
            throw new WrongAnswer("The names of the players must be different," +
        ...
        if (first == 1) {
            if (linesNonEmpty.size() != 2) { // checking a start task, program should print 2 non-empty lines
                throw new WrongAnswer("When the player provides the correct initial game conditions, "
            ...
            if (checkPencils.get(0).chars().distinct().count() != 1) {// checking any chars except |
                throw new WrongAnswer("The line with pencils should not contain any symbols other than the '|' symbol.");
            }
            if (checkPencils.get(0).length() != amount) {// checking matching the entered characters and the characters on the output
                throw new WrongAnswer("The line with pencils should contain as many '|' symbols as the player provided.");
            }

            if (!linesNonEmpty.get(1).matches(firstPlayer.toLowerCase() + "|.+turn[:!]?$")) { //Checking name and "turn!?:?"
                throw new WrongAnswer("When the player provides the initial game conditions, there should be a line in the output "
            ...
            if (botTake != 0 && turn != botTake) {// checking "winning strategy"
                throw new WrongAnswer("The " + firstPlayer + "'s move doesn't follow a winning strategy. Example:\n"
            ...
            if (linesNext.get(0).length() != amount - turn) { // checking "does not match the number of pens the player has chosen"
                throw new WrongAnswer(String.format("When the player provides the correct initial game conditions, the %s's line with pencils should contain as many '|' symbols as there is left after the %s's turn.", nextPlayer, firstPlayer));
            }
        ...
        while (onTable > 0) { // create loop for last testings block
            for (String value : incorrect) {
                ...
                if (!output.contains("possible values") || !output.contains("1") ||
                    ...
                    throw new WrongAnswer("If the player enters values different from '1', '2', or '3', " +
                            "the game should inform the user that their input is incorrect and " +
                ...
                if (!output.contains(firstPlayer.toLowerCase()) && output.contains(nextPlayer.toLowerCase())) {//checking requirements for a task if not finds correct name
                    throw new WrongAnswer("When " + firstPlayer + " provides values different from '1', '2', or '3', " +
            ...
            if (i >= onTable) {
                if (onTable != i) {
                    if (!output.contains("too many") || !output.contains("pencils")) { //checking requirements for task "too many" "pencils" were taken" if player choice > pencils
                        throw new WrongAnswer("If the player enters the number of pencils that is greater than " +
                ...
                if (linesNonEmpty.size() != 1 || !linesNonEmpty.get(0).toLowerCase().contains(nextPlayer.toLowerCase()) ||
                    ...
                    throw new WrongAnswer("When the last pencil is taken, the program should print one line that informs " +
                ...
                if (!main.isFinished()) {//finished this block and checking last output
                    throw new WrongAnswer("Your program should not request anything when there are no pencils left.");
                }
            ...
            if (onTable == 1) {
                ...
                if (checkPencils.size() != 1) {//checking task requirement "if line with pencils !=1"
                    throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, there should be exactly one line in the output for each player that contains '|'.", firstPlayer));
                }
                ...
                if (checkPencils.get(0).length() != onTable) { //checking task requirement "if after player choice pencils != correct choice pencils"
                    throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, the lines with pencils for %s should contain as many '|' symbols as there are pencils left.", firstPlayer, nextPlayer));
                }
                if (!linesPrev.get(1).toLowerCase().matches(nextPlayer.toLowerCase() + "|.+turn[:!]?$")) {//checking task requirement "match the correct sequence of string name and turn"
                    throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, there should be a line in the output for %s's turn that contains \"%s\" and \"turn\" substrings.", firstPlayer, nextPlayer, nextPlayer));
                }
                if (!turn.equals("1")) {//checking task requirement "if left 1 pencils player not takes more"
                    throw new WrongAnswer(String.format("If %s is left with 1 pencil, they can't take any other amount of pencils except for 1.", nextPlayer));
                }
                if (!result.toLowerCase().contains(firstPlayer.toLowerCase()) || (!result.toLowerCase().contains("win") && !result.toLowerCase().contains("won"))) {//checking task requirement "WHO IS WINNER?"
                    throw new WrongAnswer("When the last pencil is taken, the program should print one line that informs who is the winner in this game with \"*Name*\" and \"win\"/\"won\" strings.");
                }
            ...
            if (checkPencils.size() != 1) { //checking task requirement "each player must have one pencils output line"
                throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, "
                        + "there should be exactly one line in output for each player, that contains '|'.", firstPlayer));
            }
            if (checkPencils.get(0).chars().distinct().count() != 1) {//checking any chars except |
                throw new WrongAnswer("The pencils-lines should not contain any other symbols except the '|'.");
            }
            ...
            if (botTake != 0 && Integer.parseInt(turn) != botTake) {//checking task requirement "if bot doesn't follow a winning strategy"
                throw new WrongAnswer(String.format("The %s's move doesn't follow a winning strategy. Example:\n"
            ...
            if (checkPencils.size() != 1) {//checking task requirement "an again check output lines with pencils if != 1 for player"
                throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, there should be exactly one line in output for each player that contains '|'.", firstPlayer));
            }
            ...
            if (checkPencils.get(0).length() != onTable) {//checking task requirement "if inputs != correct number of pencils after take"
                throw new WrongAnswer(String.format("After %s inputs the number of pencils they will take, the line with pencils for %s should contain as many '|' symbols as there are pencils left.", firstPlayer, firstPlayer));
            }
